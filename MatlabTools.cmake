...
function (basis_generate_matlab_executable OUTPUT_FILE)
  ...
  if (ARGN_STARTUP)
    ...
    set (STARTUP_CODE "addpath('${STARTUP_DIR}'), ${STARTUP_COMMAND}, ")
  else ()
    set (STARTUP_CODE)
  endif ()
...
'${MATLAB_EXECUTABLE}' -nodesktop -nosplash ${ARGN_OPTIONS} \\
    -r \"try, addpath('${ARGN_MATLABPATH}', '-begin')${STARTUP_CODE}, ${ARGN_COMMAND}($args), catch err, fprintf(2, ['??? Error executing ${ARGN_COMMAND}\\n' err.message '\\n']), end, quit force\" \\
    2> >(tee \"$errlog\" >&2)"
...
function (basis_build_mcc_target TARGET_UID)
  ...
  if (EXECUTABLE AND NOT COMPILE)
    ...
    if (SOURCE_PACKAGE MATCHES "^\\+")
      ...
      set (INSTALL_STARTUP_FILE "${INSTALL_SOURCE_DIR}/${SOURCE_PACKAGE}/startup.m")
    else ()
      set (BUILD_STARTUP_FILE   "${BINARY_DIR}/startup.m")
      set (INSTALL_STARTUP_FILE "${INSTALL_SOURCE_DIR}/startup.m")
    endif ()
    # MATLAB search path
    #
    ...
    if (SOURCES MATCHES "^${BINARY_CODE_DIR_RE}")
      ...
      if (REL)
        list (APPEND BUILD_MATLABPATH "$__DIR__/${REL}")
      else ()
        list (APPEND BUILD_MATLABPATH "$__DIR__")
      endif ()
    endif ()
    list (APPEND BUILD_MATLABPATH "${SOURCE_DIR}")
    file (RELATIVE_PATH REL "${INSTALL_DIR}" "${CMAKE_INSTALL_PREFIX}/${INSTALL_SOURCE_DIR}")
    if (REL)
      list (APPEND INSTALL_MATLABPATH "$__DIR__/${REL}")
    else ()
      list (APPEND INSTALL_MATLABPATH "$__DIR__")
    endif ()
