...
function (basis_add_doc TARGET_NAME)
  ...
  elseif (GENERATOR MATCHES "SPHINX")
...
function (basis_add_sphinx_doc TARGET_NAME)
  ...
  if (NOT ARGN_SOURCE_DIRECTORY)
    set (ARGN_OUTPUT_DIRECTORY "${CMAKE_CURRENT_SOURCE_DIR}")
  ...
  if (NOT ARGN_OUTPUT_DIRECTORY)
    set (ARGN_OUTPUT_DIRECTORY "${CMAKE_CURRENT_BINARY_DIR}")
  ...
  if (ARGN_LOGO)
    ...
    configure_file ("${ARGN_LOGO}" "${BUILD_DIR}/${N}" COPYONLY)
    set (LOGO "${N}")
  else ()
    set (LOGO "None")
  endif ()
  set (PAGES)
  foreach (P IN LISTS ARGN_PAGES)
    if (PAGES)
      set (PAGES "${PAGES}, ")
    else ()
      set (PAGES "{")
  ...
  while (LEN GREATER 1)
    list (GET ARGN_UNPARSED_ARGUMENTS 0 SETTING)
    list (GET ARGN_UNPARSED_ARGUMENTS 1 VALUE)
    list (APPEND ARGS "-D" "${SETTING}=${VALUE}")
    list (REMOVE_AT ARGN_UNPARSED_ARGUMENTS 0 1)
    list (LENGTH ARGN_UNPARSED_ARGUMENTS LEN)
  endwhile ()
  if (LEN GREATER 0)
    message (FATAL_ERROR "Too many/few arguments given!")
  endif ()
  ...
  add_custom_target (
    ${TARGET_UID} "${Sphinx-build_EXECUTABLE}" -N -W -n ${ARGS}
        -c "${BUILD_DIR}"
