...
EOF-DESCRIPTION
    ...
    cat - << EOF-EXAMPLES
Examples:
  $exec_name --conf /etc/basis/testd.conf --schedule /var/run/basis/testd
...
test_cmd="$exec_dir/basistest" # command used to run tests
	...
	case "$1" in
        ...
        -t|--testcmd)
            ...
            if [ $# -gt 0 ]; then
                ...
                echo "Option -t [ --test_cmd ] requires an argument!" 1>&2
                exit 1
            fi
            ;;
        -s|--schedule)
...
if [ $? -ne 0 ]; then
    echo "Could not find test command $test_cmd" 1>&2
    exit 1
fi
...
num=${#new_schedule[@]}
while [ $idx -lt $num ]; do
    echo ${new_schedule[$idx]} >> $schedule_file
    if [ $? -ne 0 ]; then
        echo "Failed to write schedule to file $schedule_file!" 1&>2
        break
    fi
    ((idx++))
done
if [ -f $schedule_file ]; then
    sort "$schedule_file" -o "$schedule_file"
fi
