...
macro (basis_write_list FILENAME VARIABLE)
  file (WRITE "${CMAKE_FILE}" "# Automatically generated. Do not edit this file!\nset (${VARIABLE}\n")
  foreach (V IN LISTS ARGN)
    file (APPEND "${CMAKE_FILE}" "  \"${V}\"\n")
  endforeach ()
...
#
# This function globs all source files which match one of the given extensions
# (plus an optional .in suffix) within any of the specified source directories.
# It is in particular used to get a list of the scripted modules in the
# @c PROJECT_LIBRARY_DIR and the public header files in @c PROJECT_INCLUDE_DIR
# (the latter has not been refactored yet to make use of this function).
#
# @param [out] FILES Name of variable which holds the list of found files.
#                    The returned paths are absolute paths and the list is
#                    sorted in order to ensure the same order whenever this
#                    function is called as long as now files were added or
#                    removed from the source directories.
# @param [in]  ARGN  Named option arguments:
# @par
# <table border=0>
#   <tr>
#     @tp @b DIRECTORIES dir... @endtp
#     <tp>Source directories in which to look for files.</tp>
#   </tr>
#   <tr>
#     @tp @b EXTENSIONS ext... @endtp
#     <tp>Extensions of files to look for including the leading period (.).</tp>
#   </tr>
# </table>
function (basis_glob FILES)
  CMAKE_PARSE_ARGUMENTS (ARGN "" "" "DIRECTORIES;EXTENSIONS" ${ARGN})
  set (_FILES)
  if (NOT ARGN_DIRECTORIES)
    set (ARGN_DIRECTORIES "${CMAKE_CURRENT_SOURCE_DIR}")
  endif ()
  if (NOT ARGN_EXTENSION)
    message (FATAL_ERROR "Missing EXTENSIONS!")
  endif ()
  foreach (D IN LISTS ARGN_DIRECTORIES)
    set (PATTERN)
    foreach (E IN LISTS ARGN_EXTENSIONS)
      list (APPEND "${D}/*${E}")
      list (APPEND "${D}/*${E}.in")
    endforeach ()
  endforeach ()
  if (_FILES)
    ...
    list (SORT _FILES) # deterministic order
  endif ()
...
function (basis_get_target_location VAR TARGET_NAME PART)
  ...
  if (TARGET "${TARGET_UID}")
    ...
    # non-imported custom targets
      ...
      elseif (NOT TYPE MATCHES "^SCRIPT_LIBRARY$")
